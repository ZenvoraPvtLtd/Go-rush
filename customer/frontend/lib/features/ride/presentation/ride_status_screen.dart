...
class _RideStatusScreenState extends State<RideStatusScreen> {
  ...
  void initState() {
    ...
    // Simulate state machine progression: SEARCHING -> DRIVER_ASSIGNED
    if (_ride.status == RideStatus.searching || _ride.status == RideStatus.requested) {
      if (kDebugMode) {
        _mockDispatchTimer = Timer(const Duration(seconds: 3), () {
          if (mounted) {
            setState(() {
              _ride = Ride(
                rideId: _ride.rideId,
                ...
                specialInstructions: _ride.specialInstructions,
                otpCode: _ride.otpCode,
                driverName: 'Ramesh Kumar',
                driverPhone: '+91 98765 43210',
                driverVehicle: 'Maruti Suzuki Dzire',
